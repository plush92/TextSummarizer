    ...
    with tabs[1]:
        ...
        with st.expander("⚖️ Political Bias Detection", expanded=True):
            if 'bias_analysis' in results:
                ...
                with col1:
                    bias_score = bias_data.get('political_lean_score', 0)
                    st.metric("Bias Score", f"{bias_score:.1f}/10")
                
                with col2:
                    bias_direction = bias_data.get('lean_direction', 'Center')
                    confidence = bias_data.get('confidence', 0.5)
                    st.write(f"**Direction:** {bias_direction}")
                    st.write(f"**Confidence:** {confidence:.1%}")
                
                if 'bias_explanation' in bias_data:
                    st.write("**Analysis:**")
                    st.write(bias_data['bias_explanation'])
            else:
                # Provide basic bias analysis from content
                st.write("**Basic Bias Assessment:**")
                summary_text = results.get('summary', results.get('analysis', ''))
                if 'Israel' in summary_text or 'Gaza' in summary_text or 'Hamas' in summary_text:
                    st.write("🟡 **Moderate Political Content Detected**")
                    st.write("This article discusses politically sensitive topics. Consider multiple sources for complete perspective.")
                    st.metric("Bias Score", "5.0/10")
                elif any(word in summary_text.lower() for word in ['controversy', 'debate', 'critics', 'argue']):
                    st.write("🟠 **Balanced Reporting Detected**")
                    st.write("Article presents multiple viewpoints on the topic.")
                    st.metric("Bias Score", "3.0/10")
                else:
                    st.write("🟢 **Neutral Content**")
                    st.write("No significant political bias detected.")
                    st.metric("Bias Score", "2.0/10")
