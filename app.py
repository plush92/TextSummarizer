...
def display_comprehensive_results(results, settings):
    ...
    with tabs[1]:
        ...
        text_lower = summary_text.lower() if summary_text else ''
        ...
        with st.expander("⚖️ Political Bias Detection", expanded=True):
            ...
            # Always use dynamic content-based bias analysis for consistency
            text_lower = summary_text.lower() if summary_text else ''
            ...
            # Dynamic scoring based on frequency and context
            ...
            # Dynamic direction and confidence
            ...
            with col1:
                st.metric("Political Lean Score", f"{bias_score:.1f}/10")
            with col2:
            ...
            if severity == "Neutral":
                ...
                st.info(f"📊 This indicates {severity.lower()} {direction.lower()} bias in framing and emphasis.")
            
            # Confidence Interval
            margin = abs_score * 0.3
            lower_bound = bias_score - margin
            upper_bound = bias_score + margin
            st.caption(f"**Confidence:** {confidence:.0%} confident the true score lies between {lower_bound:.1f} and {upper_bound:.1f}")
            
            # Dynamic Evidence Analysis
            if summary_text:
                st.markdown("**Evidence Analysis:**")
                
                # Extract actual biased phrases from text
                sentences = summary_text.split('.')
                biased_phrases = []
                
                # Loaded language detection
                loaded_terms = ['mouthpiece', 'propaganda', 'targeting', 'silencing', 'crackdown', 'authoritarian', 
                               'terrorist', 'extremist', 'radical', 'dangerous', 'threat']
                for sentence in sentences[:10]:  # Check first 10 sentences
                    sentence_lower = sentence.lower().strip()
                    for term in loaded_terms:
                        if term in sentence_lower and len(sentence.strip()) > 10:
                            bias_type = "Loaded Language" if term in ['mouthpiece', 'propaganda', 'terrorist', 'extremist'] else "Interpretive Framing"
                ...
                authority_terms = ['human rights groups', 'experts say', 'critics argue', 'supporters claim']
                ...
                # Show top 3 examples or generic if none found
                if biased_phrases:
                    for i, evidence in enumerate(biased_phrases[:3], 1):
                        with st.container():
                            ...
                            if i < len(biased_phrases): st.markdown("---")
                else:
        ...
        with st.expander("🔍 Missing Perspectives Diagnostic"):
            ...
            if summary_text:
                ...
                for i, perspective in enumerate(missing_perspectives[:4], 1):
                    severity_icon = "🔴" if perspective['severity'] == 'Major' else "🟡" if perspective['severity'] == 'Moderate' else "🟢"
                    ...
                    if i < len(missing_perspectives): st.markdown("---")
        ...
        with st.expander("⚖️ Cross-Framing Analysis"):
            ...
            if summary_text:
                # Extract key topics and themes for dynamic framing
                text_lower = summary_text.lower()
                key_topics = []
                ...
                with col1:
                    ...
                    if 'government_action' in key_topics:
                        st.markdown("• \"Government overreach threatens democratic values\"")
                    if 'media_freedom' in key_topics:
                        st.markdown("• \"Press freedom under attack\"")
                    if 'rights_issue' in key_topics:
                        st.markdown("• \"Civil liberties endangered by new restrictions\"")
                    if 'security_concern' in key_topics:
                        st.markdown("• \"Security concerns used to justify censorship\"")
                    if not key_topics:
                        st.markdown("• \"Authority figures restrict public access to information\"")
                    
                    estimated_left_bias = -2.5 if len([t for t in key_topics if t in ['rights_issue', 'media_freedom']]) >= 2 else -1.8
                    st.markdown(f"• **Distance from Neutral:** {estimated_left_bias:.1f} (Left Lean)")
                
                with col2:
                    st.markdown("**📺 Right-Leaning Outlet Framing:**")
                    if 'security_concern' in key_topics:
                        st.markdown("• \"Government acts to protect national security\"")
                    if 'government_action' in key_topics:
                        st.markdown("• \"Leadership takes decisive action on threats\"")
                    if 'legal_matter' in key_topics:
                        st.markdown("• \"Legal measures enacted to maintain order\"")
                    if 'media_freedom' in key_topics:
                        st.markdown("• \"Media accountability measures implemented\"")
                    if not key_topics:
                        st.markdown("• \"Authorities implement necessary protective measures\"")
                    
                    estimated_right_bias = 2.3 if len([t for t in key_topics if t in ['security_concern', 'government_action']]) >= 2 else 1.6
                    st.markdown(f"• **Distance from Neutral:** +{estimated_right_bias:.1f} (Right Lean)")
                
                st.markdown("**📰 Neutral Outlet Framing:**")
                if 'government_action' in key_topics:
                    st.markdown("• \"Government implements new policy affecting [relevant sector]\"")
                if 'legal_matter' in key_topics:
                    st.markdown("• \"Legal challenges filed regarding new regulations\"")
                if 'media_freedom' in key_topics:
                    st.markdown("• \"Media organizations respond to new restrictions\"")
                if not key_topics:
                    st.markdown("• \"Officials announce new measures amid ongoing situation\"")
                
                # Compare current article position
                current_bias_magnitude = abs(bias_score) if 'bias_score' in locals() else 1.0
                if direction == "Left-leaning":
                    position_desc = f"Moderately left of center (distance: -{current_bias_magnitude:.1f})"
                elif direction == "Right-leaning":
                ...
                st.info("🔍 Unable to perform cross-framing analysis without content.")
        ...
        with st.expander("🧩 Bias Interaction Analysis"):
            ...
            if summary_text:
                ...
                has_loaded_language = any(term in text_lower for term in ['mouthpiece', 'propaganda', 'targeting', 'silencing', 'crackdown'])
                has_authority_framing = any(term in text_lower for term in ['human rights groups', 'experts say', 'critics argue'])
                has_selection_bias = summary_text.count('"') > 4  # Multiple quoted sources
            ...
            for interaction in interactions[:4]:  # Show top 4 interactions
        ...
        with st.expander("📈 Reader Takeaways & Media Literacy"):
            ...
            if summary_text:
                ...
                if any(term in text_lower for term in ['said', 'according to', 'reported']) and summary_text.count('"') > 2:
                    takeaways.append("📚 **Source Attribution:** Note whose voices are amplified and whose perspectives may be missing")
                
                if any(loaded_term in text_lower for loaded_term in ['mouthpiece', 'targeting', 'silencing', 'crackdown']):
                ...
                st.markdown("🔍 **General Guidance:** Always seek multiple sources and perspectives")
                st.markdown("📚 **Source Diversity:** Read both mainstream and alternative viewpoints")
                st.markdown("⚖️ **Critical Thinking:** Question framing choices and language selection")
                st.markdown("🌐 **Context Research:** Investigate background on complex topics")
